...
import fr.lwjgl3binding.system.Sys;
...
public class Minecraft implements IPlayerUsage {
	...
	private void startGame() throws RuntimeException {
		...
		try {
			...
			Display.create(this.displayWidth, this.displayHeight, "Minecraft 1.7.10");
		} catch (RuntimeException var7) {
			logger.error("Couldn\'t create window !", var7);
		...
		if (this.fullscreen) {
			...
			Display.setSize(this.displayWidth, this.displayHeight);
		}
		...
		if (var1 != Util.EnumOS.OSX) {
			try {
				File icon16 = this.mcDefaultResourcePack.getFile(new ResourceLocation("icons/icon_16x16.png"));
				File icon32 = this.mcDefaultResourcePack.getFile(new ResourceLocation("icons/icon_32x32.png"));

				if (icon16 != null && icon32 != null) {
					Display.setIcon(
							new ByteBuffer[] { this.getIconBytes(icon16, true), this.getIconBytes(icon32, false) });
				}
		...
		try {
			this.field_152353_at = new TwitchStream(this,
		...
		this.field_152350_aA = new SkinManager(this.renderEngine, new File(this.fileAssets, "skins"),
	...
	private ByteBuffer getIconBytes(File iconFile, boolean isIcon16) throws IOException {

		ByteBuffer iconBuffer;

		try {

			iconBuffer = ioResourceToByteBuffer(iconFile, isIcon16 ? 2048 : 4096);
		} catch (Exception e) {
	...
	public static ByteBuffer ioResourceToByteBuffer(File resource, int bufferSize) throws IOException {
	 ...
	 */
	private void loadScreen() throws RuntimeException {
		...
		try {
			this.field_152354_ay = this.renderEngine.getDynamicTextureLocation("logo",
					new DynamicTexture(ImageIO.read(this.mcDefaultResourcePack.getInputStream(locationMojangPng))));
	...
	public void toggleFullscreen() {
		try {
			...
			if (this.fullscreen) {
				...
				Display.setSize(this.tempDisplayWidth, this.tempDisplayHeight);
	...
	public void addServerStatsToSnooper(PlayerUsageSnooper p_70000_1_) {
		...
		p_70000_1_.func_152768_a("display_frequency", Integer.valueOf(Display.getFrequency()));
	...
	public void addServerTypeToSnooper(PlayerUsageSnooper p_70001_1_) {
		...
		GLCapabilities var2 = Display.getCapabilities();
			...
			if (!(this.currentScreen instanceof GuiControls)
				...
				if (Keyboard.getEventKeyState()) {
					if (var1 == this.gameSettings.field_152396_an.getKeyCode()) {
						if (this.func_152346_Z().func_152934_n()) {
							...
							if (this.theWorld != null) {
								this.ingameGUI.getChatGUI()
										.func_146227_a(new ChatComponentText("Not ready to start streaming yet!"));
							}
						} else {
						...
						this.ingameGUI.getChatGUI().func_146227_a(ScreenShotHelper.saveScreenshot(this.mcDataDir,
