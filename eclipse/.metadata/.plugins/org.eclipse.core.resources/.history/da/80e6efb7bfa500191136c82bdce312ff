...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	public EntityRenderer(Minecraft p_i45076_1_, IResourceManager p_i45076_2_) {
		...
		this.locationLightMap = p_i45076_1_.getTextureManager().getDynamicTextureLocation("lightMap",
	...
	private void orientCamera(float p_78467_1_) {
		...
		if (var2.isPlayerSleeping()) {
			...
			if (!this.mc.gameSettings.debugCamEnable) {
				Block var10 = this.mc.theWorld.getBlock(MathHelper.floor_double(var2.posX),
		...
		if (!this.mc.gameSettings.debugCamEnable) {
			GL11.glRotatef(var2.prevRotationPitch + (var2.rotationPitch - var2.prevRotationPitch) * p_78467_1_, 1.0F,
					0.0F, 0.0F);
			GL11.glRotatef(var2.prevRotationYaw + (var2.rotationYaw - var2.prevRotationYaw) * p_78467_1_ + 180.0F, 0.0F,
					1.0F, 0.0F);
		}
	...
	private void setupCameraTransform(float p_78479_1_, int p_78479_2_) {
		...
		new Matrix4f().setPerspective((float) Math.toRadians(this.getFOVModifier(p_78479_1_, true)),
				(float) this.mc.displayWidth / (float) this.mc.displayHeight, 0.05F, this.farPlaneDistance * 2.0F)
				.get(matBuf);
		GL11.glLoadMatrixf(matBuf);
		float var4;
	...
	private void renderHand(float p_78476_1_, int p_78476_2_) {
		if (this.debugViewDirection <= 0) {
			...
			FloatBuffer matBuf = BufferUtils.createFloatBuffer(16);
			new Matrix4f().setPerspective((float) Math.toRadians(this.getFOVModifier(p_78476_1_, false)),
					(float) this.mc.displayWidth / (float) this.mc.displayHeight, 0.05F, this.farPlaneDistance * 2.0F)
					.get(matBuf);
			GL11.glLoadMatrixf(matBuf);
					...
					var11.addCrashSectionCallable("Mouse location", new Callable() {
						...
						public String call() {
							return String.format("Scaled: (%d, %d). Absolute: (%d, %d)",
									new Object[] { Integer.valueOf(var16), Integer.valueOf(var17),
					...
					var11.addCrashSectionCallable("Screen size", new Callable() {
						...
						public String call() {
							return String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d",
									new Object[] { Integer.valueOf(var13.getScaledWidth()),
											Integer.valueOf(var13.getScaledHeight()),
											Integer.valueOf(EntityRenderer.this.mc.displayWidth),
											Integer.valueOf(EntityRenderer.this.mc.displayHeight),
											Integer.valueOf(var13.getScaleFactor()) });
						}
					});
	...
	private void setupFog(int p_78468_1_, float p_78468_2_) {
		...
		if (p_78468_1_ == 999) {
			GL11.glFogfv(GL11.GL_FOG_COLOR, this.setFogColorBuffer(0.0F, 0.0F, 0.0F, 1.0F));
			GL11.glFogi(GL11.GL_FOG_MODE, GL11.GL_LINEAR);
			GL11.glFogf(GL11.GL_FOG_START, 0.0F);
			GL11.glFogf(GL11.GL_FOG_END, 8.0F);

			if (Display.getCapabilities().GL_NV_fog_distance) {
